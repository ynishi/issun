...
//!
//! Core turn-based combat processing engine.

use super::{Combatant, CombatLogEntry, CombatResult, TurnBasedCombatConfig};
...
impl CombatEngine {
    ...
    pub fn add_log(&mut self, message: String) {
        ...
        if self.log.len() > self.config.max_log_entries {
            self.log.drain(0..self.log.len() - self.config.max_log_entries);
        }
    }
    ...
    pub fn process_turn_dyn(
        ...
        for attacker_ref in party.iter_mut().filter(|p| p.is_alive()) {
            if let Some(enemy_ref) = enemies.iter_mut().find(|e| e.is_alive()) {
                ...
                let result = self.combat_service.apply_attack(attacker_combatant, enemy_combatant, 1.0);

                self.add_log(format!("{} attacks {} for {} damage!", attacker_name, enemy_name, result.actual_damage));
        ...
        for enemy_ref in enemies.iter_mut().filter(|e| e.is_alive()) {
            ...
            let result = self.combat_service.apply_attack(enemy_combatant, target_combatant, damage_multiplier);

            self.add_log(format!("{} attacks {} for {} damage!", enemy_name, target_name, result.actual_damage));
        }
        ...
        if per_turn_damage > 0 {
            for member_ref in party.iter_mut().filter(|p| p.is_alive()) {
                ...
                let result = self.combat_service.apply_damage(member_combatant, per_turn_damage, None);
                self.add_log(format!("☢️ Contamination damages {} for {} HP!", member_combatant.name(), result.actual_damage));
            }
        }
        ...
        E: Combatant,
    {
        ...
        for attacker in party.iter_mut().filter(|p| p.is_alive()) {
            if let Some(enemy) = enemies.iter_mut().find(|e| e.is_alive()) {
                ...
                self.add_log(format!("{} attacks {} for {} damage!", attacker_name, enemy_name, damage));
        ...
        for (enemy_name, base_damage) in alive_enemies {
            ...
            if let Some(target) = party.iter_mut().find(|p| p.name() == target_name) {
                ...
                self.add_log(format!("{} attacks {} for {} damage!", enemy_name, target_name, damage));
        ...
        if per_turn_damage > 0 {
            for member in party.iter_mut().filter(|p| p.is_alive()) {
                ...
                self.add_log(format!("☢️ Contamination damages {} for {} HP!", member.name(), per_turn_damage));
            }
        }
