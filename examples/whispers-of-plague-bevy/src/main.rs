                ...
                match current_state {
                    GameScene::Title => match key.code {
                        KeyCode::Char('1') => {
                            ...
                            }
                            {
                                ...
                                ui_state.add_message("Plague mode selected".to_string());
                            }
                            {
                        ...
                        KeyCode::Char('2') => {
                            ...
                            {
                                let mut ui_state = world.resource_mut::<UIState>();
                                ui_state.add_message("Savior mode selected".to_string());
                            }
                            {
                                let mut next_state = world.resource_mut::<NextState<GameScene>>();
                                next_state.set(GameScene::Game);
                    ...
                    GameScene::Game => match key.code {
                        KeyCode::Char('n') | KeyCode::Char('N') => {
                            let message = {
                                ...
                                format!(
                                    "Turn {}/{}",
                                    game_context.turn, game_context.max_turns
                                )
                            };
                            let mut ui_state = world.resource_mut::<UIState>();
                        ...
                        // Isolation policy (reduce infection spread)
                        KeyCode::Char('i') | KeyCode::Char('I') => {
                            ...
                            if let Some(mut district) = districts.get_mut(selected) {
                                ...
                                let reduction = (district.infected as f32 * 0.1) as u32;
                                district.infected = district.infected.saturating_sub(reduction);
                                let msg = format!(
                                    "Isolation in {}! Reduced {} infections",
                                    district.name,
                                    reduction
                                );
                                let mut ui_state = world.resource_mut::<UIState>();
                    ...
                    GameScene::Result => match key.code {
                        KeyCode::Enter => {
                            ...
                            {
                                let mut query = world.query::<&mut District>();
                                for mut district in query.iter_mut(world) {
                                    district.infected = 0;
                                    district.dead = 0;
